...
export interface GuessResult {
    guess: number;
    actual: number;
    /** Absolute rupee difference. */
    difference: number;
    /** How far off, as a percentage of actual. */
    errorPercent: number;
    /** True when the learner guessed below what they actually spent. */
    underestimated: boolean;
    /** 0 to 100, where 100 is exact. */
    accuracy: number;
    grade: 'spot on' | 'close' | 'off' | 'way off';
}

/**
 * Score a spend guess.
 *
 * Accuracy falls linearly with the relative error and hits zero at 100% off,
 * so guessing double or guessing nothing both score zero.
 */
export function scoreGuess(guess: number, actual: number): GuessResult {
    const difference = Math.abs(guess - actual);
    const errorPercent = actual === 0 ? (guess === 0 ? 0 : 100) : (difference / actual) * 100;
    const accuracy = Math.max(0, Math.round(100 - errorPercent));

    const grade: GuessResult['grade'] =
        errorPercent <= 5 ? 'spot on'
            : errorPercent <= 15 ? 'close'
                : errorPercent <= 35 ? 'off'
                    : 'way off';
